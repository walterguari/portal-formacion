...
@st.cache_data(ttl=0) # ttl=0 obliga a actualizar siempre
def load_data():
    try:
        df = pd.read_csv(URL)
        ...
        # 2. RENOMBRADO INTELIGENTE (Aquí estaba el problema antes)
        rename_dict = {}
        for col in df.columns:
            # Buscamos cualquier variación de "SECTOR"
            ...
            elif "CAPACITA" in col or "ESTADO" in col: rename_dict[col] = 'ESTADO_NUM'
        ...
        # Convertir a texto y mayúsculas para evitar duplicados
        for c in ['SECTOR', 'CARGO', 'COLABORADOR']:
            if c in df.columns:
...
# =========================================================
# 🏗️ BARRA LATERAL (Aquí van tus botones)
# =========================================================
...
if df is not None and 'SECTOR' in df.columns:
    ...
    if st.sidebar.button("VER TODO", type=tipo_todos):
    ...
    for sec in sectores:
        ...
        # Layout del botón (Indicador + Texto)
        ...
        with c2:
            ...
            if st.button(f"{sec} ({porc:.0f}%)", key=sec, type=tipo):
    ...
    # 4. FILTRO DE ROL (Filtrado por sector)
    st.sidebar.header("👮 Roles / Puestos")
    ...
    # Recuperar índice para que no se resetee feo
    idx = 0
...
if not df_main.empty:
    ...
    if st.button(f"👥 Ver Todo el Equipo ({len(nombres)})", type="primary" if st.session_state.colaborador_activo == "Todos" else "secondary"):
        st.session_state.colaborador_activo = "Todos"
        st.rerun()
    ...
    for i, nom in enumerate(nombres):
        ...
        if cols[i % 4].button(nom, key=f"user_{i}", type=tipo):
